...
class ActionModule(ActionBase):
    ...
    _VALID_ARGS = frozenset(('connect_timeout', 'msg', 'boot_delay', 'post_reboot_delay', 'pre_reboot_delay', 'test_command', 'reboot_timeout', 'search_paths'))

    DEFAULT_REBOOT_TIMEOUT = 600
    DEFAULT_CONNECT_TIMEOUT = None
    DEFAULT_PRE_REBOOT_DELAY = 0
    DEFAULT_BOOT_DELAY = 180
    DEFAULT_POST_REBOOT_DELAY = 0
    DEFAULT_TEST_COMMAND = 'whoami'
    DEFAULT_BOOT_TIME_COMMAND = 'cat /proc/sys/kernel/random/boot_id'
    DEFAULT_REBOOT_MESSAGE = 'Reboot initiated by Ansible'
    DEFAULT_SHUTDOWN_COMMAND = 'shutdown'
    DEFAULT_SHUTDOWN_COMMAND_ARGS = '{delay_min} "{message}"'
    ...
    @property
    def boot_delay(self):
        return self._check_delay('boot_delay', self.DEFAULT_BOOT_DELAY)
    ...
    def perform_reboot(self, task_vars, distribution):
        ...
        try:
            ...
            self._low_level_execute_command("echo 0 > /sys/class/rtc/rtc0/wakealarm", sudoable=self.DEFAULT_SUDOABLE)
            self._low_level_execute_command("echo `date '+%s' -d '+ 3 minutes'` > /sys/class/rtc/rtc0/wakealarm", sudoable=self.DEFAULT_SUDOABLE)
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if distribution != 'linux':
            msg = 'Only working on linux platform'.format(self._task.action)
            return {'changed': False, 'elapsed': 0, 'rebooted': False, 'failed': True, 'msg': msg}
