...
echo "MMMM MMMMMMMMMMM :MMMMMMMMMM     ====                                         "
echo "MMMMMMMMMMMMMMMMMMMMMMMMMMMMM                                                 "
echo "MMMMMMM   .MMMMMMMM    MMMMMM                                                 "
echo "MMMMM.      MMMMMM      MMMMM                                                 "
echo "MMMMM       MMMMM       MMMMM                                                 "
echo "MMMMM       MMMMM       MMMMM   M   M M    .M MMMM, MMMMM MMMM, M  .MM,  M   M"
echo "MMMMM       MMMMM       MMMMM   M   M  M  .M  M   M M     M   M M  M  M  MM  M"
echo "MMMMM       MMMMM       MMMMM   M   M   M M   M  .M M     M   M M M    M MM, M"
echo "MMMMM       MMMMM       MMMMM   MMMMM    M    MMMM  MMMM  MMMM  M M    M M M M"
echo "MMMMM       MMMMM       MMMMM   M   M    M    M     M     MM    M M,  .M M  MM"
echo "MMMMM       MMMMM       MMMMM   M   M    M    M     M     M M   M  M,.M  M   M"
echo "MMMMM       MMMMM       MMMMM   M   M    M    M     MMMMM M  M  M   MM   M   M"
...
# Set variables for mvIMPACT_acquire for later use
if grep -q '/etc/ld.so.conf.d/' /etc/ld.so.conf; then
    ACQUIRE_LDSOCONF_FILE=/etc/ld.so.conf.d/acquire.conf
    HYPERION_LDSOCONF_FILE=/etc/ld.so.conf.d/hyperion.conf
else
    ACQUIRE_LDSOCONF_FILE=/etc/ld.so.conf
    HYPERION_LDSOCONF_FILE=/etc/ld.so.conf
fi
...
echo
echo "${bold}Camera definition directory:       ${reset}"$DEF_DATA_DIRECTORY/camerafiles
echo "${bold}ldconfig:"
echo "${bold}mvHYPERION:                     ${reset}"$HYPERION_LDSOCONF_FILE
...
if grep -q '/etc/ld.so.conf.d/' /etc/ld.so.conf; then
    ...
    $SUDO rm -f $HYPERION_LDSOCONF_FILE
    $SUDO touch $HYPERION_LDSOCONF_FILE
fi
    ...
    if ([ "$REPOSITORY_ACCESS_AVAILABLE" == "YES" ] || [ "$FORCE_REPOSITORY_USAGE" == "YES" ]) || [ "$USE_DEFAULTS" == "YES" ]; then
        ...
        if [ "$(wx-config --release 2>&1 | grep -c "^3.")" != "1" ] ||
            ...
            if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
                ...
                if [ "$OS_NAME" == "Ubuntu" ] || [ "$OS_NAME" == "Debian" ] || [ "$OS_NAME" == "Linuxmint" ]; then
                    REQUIRED_PACKAGES=(libwxgtk-webview3.2-dev libwxgtk3.2-dev  wx3.2-headers libgtk2.0-dev)
                    installPackages "${REQUIRED_PACKAGES[@]}"
...
echo
echo "Do you want to copy 50-udev-hyperion.rules to /etc/udev/rules.d for non-root user support (default is 'yes')?"
echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
if [ "$USE_DEFAULTS" == "NO" ]; then
    read YES_NO
else
...
if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
    ...
    echo 'copy 50-udev-hyperion.rules the file to /etc/udev/rules.d'
    echo
else
    $SUDO cp -f $DEF_DIRECTORY/Scripts/50-udev-hyperion.rules /etc/udev/rules.d
fi

# Modifying settings in 50-udev-hyperion.rules to make sure every user can use the framegrabber
if ! [ -f /etc/udev/rules.d/50-udev-hyperion.rules ]; then
    echo '/etc/udev/rules.d/50-udev-hyperion.rules is not available on your system'
    echo 'without this file the framegrabber will not be acessible'
    exit
else
    $SUDO sed -i "s/.*GROUP=\"video\".*/#KERNEL==\"hyperion-cl[0-9]*\",  NAME=\"hyperion-cl%n\", GROUP=\"video\"/g" /etc/udev/rules.d/50-udev-hyperion.rules
    $SUDO sed -i "s/.*MODE=\"0666\".*/KERNEL==\"hyperion-cl[0-9]*\",  NAME=\"hyperion-cl%n\", MODE=\"0666\"/g" /etc/udev/rules.d/50-udev-hyperion.rules
fi

# reload the udev rules after change.
$SUDO $UDEVADM control --reload-rules

# building kernel module for HYPERION
echo
echo "Do you want the mvHYPERION kernel module to be built (default is 'yes')?"
echo "The kernel module MUST be built for the frame grabber to be usable!"
echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
if [ "$USE_DEFAULTS" == "NO" ]; then
    read YES_NO
else
...
if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ] || [ "$SKIP_KERNEL_MODULE" == "YES" ]; then
    echo
    echo "Please restart the installer and select 'YES' if you want to use the frame grabber!"
    ...
    if [ -r $DEF_DIRECTORY ]; then
        ...
        echo ""
        echo "**********************************************************"
        cd $DEF_DIRECTORY/kernelmodules/linux/Hyperion/
        ./configure >/dev/null
        cd $DEF_DIRECTORY/kernelmodules/linux/Hyperion2/
        ./configure >/dev/null
        cd $DEF_DIRECTORY/kernelmodules/linux/HyperionGeneric
        ./configure >/dev/null
        cd $DEF_DIRECTORY
        $SUDO make install
        if [ $? -ne 0 ]; then
...
echo
# check if video group exists and the user is member of it
if [ "$(grep -c ^video: /etc/group)" == "0" ]; then
    echo "Group 'video' doesn't exist, this is necessary to use HYPERION devices as a normal user,"
    echo "do you want to create it and add current user to 'video' (default is 'yes')?"
    echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
    if [ "$USE_DEFAULTS" == "NO" ]; then
        read YES_NO
    else
    ...
    if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
        echo
        echo "'video' will be not created and you can't run the device as non-root user!"
        echo "If you want non-root users support, you will need to create 'video'"
        echo "and add the users to this group."
    else
        $SUDO /usr/sbin/groupadd -g 46 video
        $SUDO /usr/sbin/usermod -a -G video $USER
        echo "Group 'video' created and user '"$USER"' added to it."
    fi
else
    if [ "$(groups | grep -c video)" == "0" ]; then
        echo "Group 'video' exists, however user '"$USER"' is not a member, which is necessary to"
        echo "use HYPERION devices. Do you want to add  user '"$USER"' to 'video' (default is 'yes')?"
        echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
        if [ "$USE_DEFAULTS" == "NO" ]; then
            read YES_NO
        else
        ...
        if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
            echo
            echo "If you want to use HYPERION devices you have to manually add user '"$USER"' to the video group."
            ((WARNING_NUMBER = WARNING_NUMBER + 1))
        else
            $SUDO /usr/sbin/usermod -a -G video $USER
...
# create the camera definitions directory
if ! [ -d $DEF_DATA_DIRECTORY/camerafiles ]; then
    mkdir -p $DEF_DATA_DIRECTORY/camerafiles
    if ! [ -d $DEF_DATA_DIRECTORY/camerafiles ]; then
        # that didn't work, now try it as superuser
        $SUDO mkdir -p $DEF_DATA_DIRECTORY/camerafiles
    fi
    if ! [ -d $DEF_DATA_DIRECTORY/camerafiles ]; then
        echo 'ERROR: Could not create camera definition directory' $DEF_DATA_DIRECTORY/camerafiles '.'
        echo 'Problem:'$?
        echo 'Maybe you specified a partition that was mounted read only?'
        echo
        exit
...
if [ -d /etc/security/limits.d ]; then
    if [[ ! -f /etc/security/limits.d/acquire.conf || "$(grep -c '@video              -       nice            -20' /etc/security/limits.d/acquire.conf)" == "0" ]]; then
        echo '@video              -       nice            -20' | sudo tee -a /etc/security/limits.d/acquire.conf >/dev/null
    fi
    if [ "$(grep -c '@video              -       rtprio          99' /etc/security/limits.d/acquire.conf)" == "0" ]; then
        echo '@video              -       rtprio          99' | sudo tee -a /etc/security/limits.d/acquire.conf >/dev/null
    fi
else
